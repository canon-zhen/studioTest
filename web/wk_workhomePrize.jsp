            ...
            <div class="all_success">
                <div class="success_topx">
                    ...
                    <div class="search_bar">
                        <form method="post" action="${pageContext.request.contextPath}/queryAchievementBynameServlet?role=1">
                            <input type="text" name="wname">
                            <button type="submit"></button>
                        </form>
                    </div>
                </div>
                <div class="chengjiulist">
                    <table class="allList">
                        <thead>
                        <tr>
                            <th>工作室</th>
                        ...
                        <c:if test="${empty achievements}">
                            <tr>
                                <td rowspan="9" colspan="9">
                                    <a style="color: #ff7676">查询工作室成就信息</a>
                                </td>
                            </tr>
                        </c:if>
                        <c:if test="${not empty achievements}">
                            <c:forEach var="achievement" items="${achievements}" varStatus="s" >
                                <tr>
                                    <td>${achievement.wkHname}</td>
                                    <td>${achievement.achName}</td>
                                    <td>${achievement.achTime}</td>
                                    <td>${achievement.achTeacher}</td>
                                </tr>
                            </c:forEach>
                        </c:if>
                        </tbody>
